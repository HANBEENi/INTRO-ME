...
const D3Chart2 = ({ data, chartId }:D3ChartProps) => {
  ...
  useEffect(() => {
    ...
    data.forEach((d, i) => {
        const grape = svg.append("path")
            ...
            .on("mouseover", function() {
                ...
                svg.selectAll("text.category-label").style("opacity",0.2).style("font-size","1rem");

                // 현재 막대의 opacity 1로 설정
                d3.select(this as any).style("opacity",1).transition().duration(200);
                // 툴팁 블럭 보이게 설정
                d3.select(`#tooltip-${i}`).transition().duration(300).style("display","block"); 
                // 현재 카테고리-라벨의 opacity 1로 설정
                svg.selectAll(`text.category-label`).filter((datum, index) => index === i).style("opacity", 1).style("font-size","1.5625rem");
                ...
                // 하단텍스트
                bottomText.transition().duration(400)
                  .style("opacity", 1)
                  .style("fill", `${d.color}`)
            ...
            .on("mouseout", function(){
                ...
                svg.selectAll("text.category-label").style("opacity",1).style("font-size","1rem");

                d3.select(this as any).transition().duration(200);
                d3.select(`#tooltip-${i}`).transition().duration(300).style("display","none");
        ...
        grape.transition()
            ...
            .attrTween("d", function (d: any) {
              const interpolate = d3.interpolate((this as any)._current, { startAngle: 0, endAngle: (d.value / 118) * 2 * Math.PI });
              return (t: number) => {
    ...
    /** 카테고리 라벨 추가 */
    const categoryLabels = (data: DataItem[], svg: d3.Selection<SVGGElement, unknown, HTMLElement, any>, innerRadius: number)=>{
        const category = svg.selectAll("text.category-label")
            .data(data)
            .enter()
            .append("text")
            .attr("class", "category-label")
            .attr("x", -15)
            .attr("y", (d, i) => (innerRadius + i * 20 + 18) * Math.sin(-Math.PI / 2))
            ...
            .on("mouseover", function(event, datum){
              ...
              d3.select(svg.selectAll("path").nodes()[index]).style("opacity", 1).transition().duration(200);
              svg.selectAll(`text.category-label`).filter((_, i) => i === index).style("opacity", 1).style("font-size", "1.5625rem");
              centerText.transition().duration(400)
                  .style("opacity", 1)
